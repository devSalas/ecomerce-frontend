...
const FormPay = () => {
  ...
  const handleSubmit =async (evt)=>{
    ...
    if(!error ){ 
      ...
      const {data}=await axios.post("http://localhost:3000/",{...paymentMethod,amount: RoundedPriceTotal,products });
  ...
  return (<>
    <div className=" max-w-xs  flex flex-col justify-between   bg-white border-2 m-auto p-2 border-slate-400 rounded-2xl sm:mt-20   ">
      ...
      <h2 className='text-center mb-4 font-bold text-2xl p-2 text-purple-500 border-b-2 border-black'>Metodo de Pago</h2>

      {!isAuthenticated && <div className='text-center bg-red-400 p-2 rounded-md text-sm mb-4'>necesitas registarte para poder seguir</div>}
      ...
      <form  onSubmit={handleSubmit} className='border-slate-700 grow '>
        ...
        <div className='flex justify-between'>
          <div>
            <label htmlFor="expiracion" className='my-2'>
              ...
              <CardExpiryElement id="expiracion" className='border-2 p-3 rounded-md  w-20 my-2 bg-teal-200'/>
          </div>
          <div>
            <label htmlFor="expiracion" className='my-2'>
            CVC 
        ...
        <button   className={`${!isAuthenticated ?'opacity-50':''} h-12 bg-blue-400 w-full  rounded-md my-4 text-white font-bold text-xl`}>Pagar ahora</button>
